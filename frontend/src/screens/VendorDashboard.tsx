...
import { Search, Filter, Package, Users, ShoppingCart as CartIcon, Shield, User as UserIcon } from 'lucide-react';
...
const VendorDashboard: React.FC<VendorDashboardProps> = ({ userType, currentView, setCurrentView, cartItems, addToCart }) => {
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'products':
        return (<div className="p-4"><div className="flex space-x-2 mb-4"><div className="flex-1 relative"><Search className="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input type="text" placeholder="Search products..." className="w-full pl-10 pr-4 py-2 border rounded-lg" /></div><button className="p-2 border rounded-lg"><Filter className="w-5 h-5" /></button></div><div className="space-y-3">{sampleProducts.map(p => <ProductCard key={p.id} product={p} onAddToCart={addToCart} />)}</div></div>);
      case 'orders':
      ...
      case 'profile':
        return (<div className="p-4"><div className="bg-white rounded-lg shadow-sm p-4 mb-4"><div className="flex items-center space-x-4"><div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center"><UserIcon className="w-8 h-8 text-green-600" /></div><div><h3 className="font-semibold text-lg">Rajesh Kumar</h3><p className="text-sm text-gray-600">Street Vendor • Greater Noida</p><div className="flex items-center space-x-1 mt-1 text-green-600"><Shield className="w-4 h-4" /><span className="text-sm font-medium">Verified</span></div></div></div></div><TrustScore score={92} badges={['Verified Vendor', 'Regular Customer', '50+ Orders', 'Good Payment History']} /></div>);
      default:
